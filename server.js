...
app.get('/api/truck-violations/:date', requireAuth, async (req, res) => {
    try {
        ...
        const result = await pool.query(`SELECT truck_number, driver_name, trips_count, reason, details FROM truck_violations WHERE report_date = $1 ORDER BY truck_number`, [date]);
...
app.get('/api/truck-violations/report/:startDate/:endDate', requireAuth, async (req, res) => {
    try {
        ...
        const result = await pool.query(`SELECT * FROM truck_violations WHERE report_date BETWEEN $1 AND $2 ORDER BY report_date DESC, truck_number`, [startDate, endDate]);
        res.json(result.rows);
    } catch (e) {
        res.status(500).json({ error: e.message });
    }
...
app.get('/api/truck-violations/stats/:startDate/:endDate', requireAuth, async (req, res) => {
    try {
        ...
        const stats = await pool.query(`
            SELECT COUNT(DISTINCT truck_number) as total_trucks, COUNT(*) as total_violations, COALESCE(AVG(trips_count),0) as avg_trips, SUM(CASE WHEN trips_count=0 THEN 1 ELSE 0 END) as zero_trips_count
            FROM truck_violations WHERE report_date BETWEEN $1 AND $2
        `, [startDate, endDate]);
        const topTrucks = await pool.query(`
            ...
            FROM truck_violations WHERE report_date BETWEEN $1 AND $2
            GROUP BY truck_number, driver_name ORDER BY violation_count DESC LIMIT 10
        `, [startDate, endDate]);
        const topReasons = await pool.query(`
            SELECT reason, COUNT(*) as count FROM truck_violations WHERE report_date BETWEEN $1 AND $2 AND reason != '' GROUP BY reason ORDER BY count DESC LIMIT 10
        `, [startDate, endDate]);
        res.json({ general: stats.rows[0], topTrucks: topTrucks.rows, topReasons: topReasons.rows });
    } catch (e) {
        res.status(500).json({ error: e.message });
    }
});
